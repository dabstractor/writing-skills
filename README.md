...
- **[`write-pull-request`](skills/write-pull-request/SKILL.md)**: writes a pull request to an open source repo. Leads with the user-facing problem in a personal voice, then a scannable technical fix. Reads the linked issue first if one exists, and matches its phrasing. Terse imperative title (commit-message style), markdown welcome, no em dashes, no marketing tell-words. Ships [`scripts/lint.sh`](skills/write-pull-request/scripts/lint.sh).
